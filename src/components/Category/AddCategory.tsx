...
const AddCategory = () => {
    ...
    const mutation = useMutation({
        mutationFn: (data: typeof initialFormData) => createNewCategoryApi(data as any),
        onSuccess: (data: any) => {
            if (data.status === 0) {
        ...
        <>
            <div className='flex items-center pb-5'>
                ...
                <div className='flex flex-1 justify-center pr-12'>
                    <span className='text-2xl font-semibold'>Tạo danh mục mới</span>
                </div>
            </div>
                ...
                <div className='flex flex-col justify-center items-center'>
                    <div className='h-48 w-48 ring-1 ring-inset ring-gray-300 flex rounded items-center justify-center text-2xl font-semibold bg-slate-100 relative'>
                        {formik.values.previewImgURL ? (
                            <img src={formik.values.previewImgURL} alt='Avatar' className='h-full w-full rounded p-4' />
                        ) : (
